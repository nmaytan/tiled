...
async def _insert_node(store, node_id, tags, key="node", parent=None):
    async with store._engine.begin() as conn:
        ...
        tag_ids = await _tag_ids(conn, tags)
        assert len(tag_ids) == len(set(tags)), f"undefined tags among {tags}"
        if tag_ids:
            await conn.execute(
                sa_insert(_node_access_tags),
                [{"node_id": node_id, "tag_id": tag_id} for tag_id in tag_ids],
            )
...
async def test_update_entity_detaching_node_reinitializes_access_tags(store, policy):
    """
    Detaching node_id (setting it to null) with no access tags supplied in
    the same call must not leave the entity with node_id=None AND no tags
    of its own -- that would make it invisible to everyone.
    """
    await _insert_node(store, 1, ["team"])
    alice_ctx = _context(store, policy, "alice", {"read:metadata", "write:metadata"})
